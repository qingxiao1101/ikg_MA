...
class RANSAC():
    ...
    def run(self, xyzs, inlier_thres = 0.7, max_iterations=50):
        #print(max_iterations)
        n = len(xyzs)
        goal_inliers = n * inlier_thres
    
        # RANSAC
        m, mask, run_times = self.run_ransac(xyzs, lambda x, y: self.is_inlier(x, y, 0.075), 30, goal_inliers, max_iterations)
        a, b, c, d = m
