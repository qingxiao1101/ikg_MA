# -*- coding: utf-8 -*-
'''
perform the scan line method to get the pseudo labels
'''
...
import utils
from draw_train_label import draw as draw_label
import pandas as pd

import scipy as sp
import scipy.ndimage

root = '/Volumes/Qing Xiao/ikg/3_image_generation/train/' 
bias = 0.05
...
def reference_averages(img, density_image, horizontal, vertical, global_avg, bg, modality='median'):
    ...
    avg_density = np.zeros(img.shape)
    #bg = img.min()
        ...
        for j in range(len(vertical)-1):
            ...
            if len(idx)==0:
                ...
                if a < 5:
                    ...
                    b = 2
            avg_density[horizontal[i]:horizontal[i+1], vertical[j]:vertical[j+1]] = b
    return averages, avg_density



def refine_divide(ref_image, horizontal_0, vertical_0):
...
def cluster_method(lines, geo_img, im_outer_rect, scann_width):
    if len(lines) == 0:
        return geo_img, im_outer_rect
    
    draw = ImageDraw.Draw(geo_img) 
    test = [(i[0],i[1]) for i in lines]
    test_lines = sorted(lines, key=lambda x:x[2])
    test_lines = np.array(test_lines)
    dbscan = DBSCAN(eps = 3, min_samples = 5)
    pred = dbscan.fit_predict(test)
    pred = np.array(pred)
    #fig = plt.figure()
    #ax = Axes3D(fig)
    for i in range(max(pred)):
        idx = np.where(pred==i)[0]
        
        cur = test_lines[idx]
        sub_cur = [[cur[0]]]
        for j in range(1, len(cur)):
            if cur[j, 2] - cur[j-1,2] < 15:
                sub_cur[-1].append(cur[j])
            else:
                sub_cur.append([])
        for sub in sub_cur:
            if len(sub)>10:
                ...
                idx_h_min = hs.argmin()
                idx_h_max = hs.argmax()
                leftup = (min(sub[idx_h_min,0], sub[idx_h_max,0]),sub[idx_h_min,2])
                rightdown = (max(sub[idx_h_min,1], sub[idx_h_max,1]), sub[idx_h_max,2])
                ...
                if w*0.02 >6.0 or h*0.02>4.0:
                    continue
                if w/h >= 5 or w/h < 0.02:
                    continue
        
                cv2.rectangle(im_outer_rect, leftup, rightdown, (0, 255, 0)) 
                for s,e,h in sub:
                    draw.line((s,h)+(e+scann_width,h),fill=255)
                #ax.scatter(sub[:,0], sub[:, 1], sub[:,2], marker='.')
...
def processing(path, geo_path, depth_path, density_path, info_path, save_path, nid):
    ...
    final_rects = utils.prediction_filter(final_rects, candidates, iou=0.3)
    ...
    train_labels = utils.prediction_filter(predictions, candidates, iou=0.8)
    path_image_train_label = os.path.join(save_path, 'image_train_label')
    if not os.path.exists(path_image_train_label):
        os.makedirs(path_image_train_label)   
    
    train_label_coco = utils.label_convert_coco(train_labels, columns, rows)
    
    draw_label(depth_path, path_image_train_label, predictions, train_labels, nid)
    path_txt_train_label = os.path.join(save_path, 'txt_train_label')
    if not os.path.exists(path_txt_train_label):
        os.makedirs(path_txt_train_label)
    txt_file = os.path.join(path_txt_train_label, '{}.txt'.format(nid))
...
def process_all(root):
    ...
    save_path_root = os.path.join(root, 'labels')
    ...
    for idx, file in enumerate(dat_files):
        ...
        processing(path, geo_path, depth_path,density_path, info_path, save_path_root, nid)
